...
function Vertex({ data, EdgeCreateEnd, EdgeCreateStart,deleteVertex,stack }) {
    ...
    return (
        <div onDoubleClick={(e)=>{
            ...
            deleteVertex?.({index})
            }} className={`z-50 cursor-pointer flex items-center justify-center h-8 w-8 rounded-full border-2 ${current_vertex?.visited?.[current_vertex?.visited?.length-1]==index?"border-red-500":current_vertex?.visited==undefined?"border-gray-400":   current_vertex?.visited?.indexOf(`${index}`)==-1?"border-gray-400":"border-blue-500"   }  bg-white text-gray-400 hover:text-purple-400 hover:border-purple-400 absolute -translate-x-1/2 -translate-y-1/2`} style={{
